...
genesis_register_sidebar( array (
	' . $single_quote . 'name' . $single_quote . '			=>	__( ' . $single_quote . $widget_bits['widget_name'] . $single_quote . ', ' . $single_quote . 'dynamik' . $single_quote . ' ),
	' . $single_quote . 'id' . $single_quote . ' 			=>	' . $single_quote . $widget_bits['widget_name'] . $single_quote . '
) );
';
		}
	}
...
';
			...
			{
				$dynamik_widget_areas .= '
add_shortcode( ' . $single_quote . $widget_bits['widget_name'] . $single_quote . ', ' . $single_quote . 'dynamik_' . $widget_bits['widget_name'] . '_widget_area_shortcode' . $single_quote . ' );
function dynamik_' . $widget_bits['widget_name'] . '_widget_area_shortcode() {';
				$dynamik_widget_areas .= '
	ob_start();
	dynamik_' . $widget_bits['widget_name'] . '_widget_area_content();
...
';
			...
			{
				$dynamik_widget_areas .= '
add_action( ' . $single_quote . $widget_bits['hook_location'] . $single_quote . ', ' . $single_quote . 'dynamik_' . $widget_bits['widget_name'] . '_widget_area' . $single_quote . ', ' . $widget_bits['priority'] . ' );
function dynamik_' . $widget_bits['widget_name'] . '_widget_area() {';
				$dynamik_widget_areas .= '
	dynamik_' . $widget_bits['widget_name'] . '_widget_area_content();
}
';
			...
			$dynamik_widget_areas .= '
function dynamik_' . $widget_bits['widget_name'] . '_widget_area_content() {';
	...
	if ( ' . substr( $dynamik_conditional_tags, 0, -4 ) . ' ) {';
			...
			$dynamik_widget_areas .= '
	' . $tab . 'genesis_widget_area( ' . $single_quote . $widget_bits['widget_name'] . $single_quote . ', $args = array (
		' . $tab . $single_quote . 'before' . $single_quote . '              => ' . $single_quote . '<div id="' . $widget_bits['widget_name'] . '" class="widget-area dynamik-widget-area' . $widget_bits['class'] . '">' . $single_quote . ',
		' . $tab . $single_quote . 'after' . $single_quote . '               => ' . $single_quote . '</div>' . $single_quote . ',
		' . $tab . $single_quote . 'before_sidebar_hook' . $single_quote . ' => ' . $single_quote . 'genesis_before_' . $widget_bits['widget_name'] . '_widget_area' . $single_quote . ',
		' . $tab . $single_quote . 'after_sidebar_hook' . $single_quote . '  => ' . $single_quote . 'genesis_after_' . $widget_bits['widget_name'] . '_widget_area' . $single_quote . '
	' . $tab . ') );';
...
function dynamik_get_widgets()
{
	...
	if( !empty( $custom_widgets ) )
	{
		...
		$custom_widgets = dynamik_array_sort( $custom_widgets, 'widget_name' );	
 ...
 * @since 1.0
 */
function dynamik_update_widgets( $names = '', $conditionals = '', $hooks = '', $classes = '', $statuses = '', $priorities = '' )
{
	$dynamik_widgets = get_option( 'dynamik_gen_custom_widget_areas' );
	$these_widgets = array();
	...
	if( !empty( $these_widgets ) )
	{
		...
		{
			$dynamik_widgets = get_option( 'dynamik_gen_custom_widget_areas' );
			$widget_id = $widget_name = $this_widget['name'];
			...
			if( !empty( $widget_id ) )
			{
				$new_values = array( $widget_id => array( 'widget_name' => $widget_name, 'conditionals' => $conditionals, 'hook_location' => $hook_location, 'class' => $class, 'status' => $status, 'priority' => $priority ) );
...
//end lib/functions/dynamik-widget-areas.php